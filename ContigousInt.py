...
noOfTestCases = input();
for eachCase in range(int(noOfTestCases)):
    noOfElemInArr = input();
    strArr = input();
    numberList = strArr.split();
    numbers = [ int(x) for x in numberList ];
    numbers.sort();
    print(numbers);
    arrSortedSet = list(set(numbers));
    isContingous = True;
    for i in range(len(arrSortedSet)-1):
        if int(arrSortedSet[i+1]) - int(arrSortedSet[i]) != 1 :
            isContingous = False;
    
    if isContingous == True:
        print("YES");
    else :
        print("NO");
